...
describe("fora", () => {
  ...
  it(" Advisor_Bookings_TC008", async () => {
    ...
    await close.click();
    await browser.pause(5000);
    const showbookings = await $(
      '//*[@id="main-container"]/main/div/div[1]/div[2]/div/div/div/div/div/div/div[3]/div[1]/div[2]/div[1]/div[1]/div/p'
    );
  ...
  it.only(" Advisor_Bookings_TC016", async () => {
    ...
    const elements = await $(
      'android=new UiSelector().className("bg-white border border-stroke rounded-md shadow-card")'
    );
    ...
    const tb1 = await $(
      '//*[@id="main-container"]/main/div/div[1]/div[3]/div/div/div/div/div/div/div[2]/div/div/div[1]/div'
    );
...
*/
 ...
 booking.click()*/
    ...
    const manage = await $(
      "(//span[@class='text-link cursor-pointer mr-8'][normalize-space()='Manage booking'])[2]"
    );
    manage.click();
    await browser.pause(10000);
    ...
    //text items
    const managebooking = await $("//div[@class='flex flex-col p-6']");
    const textsToCheck = [
      "Cancel booking",
      "Report cancellation",
      "Modify booking",
 ...
 booking.click()*/
  ...
  for (const text of textsToCheck) {
    ...
    const cmt = await $(
      "(//textarea[@placeholder='Please provide additional details/notes.'])[1]"
    );
    ...
    const reason = await $(
      '//*[@id="headlessui-dialog-panel-:rv:"]/div[1]/div/div[2]/div[2]/div/label'
    );
    ...
    const dropdwnOptions = await $(
      '//*[@id="headlessui-dialog-panel-:rv:"]/div[1]'
    );
    ...
    const entireTrip = await $(
      "//div[normalize-space()='Entire trip canceled']"
    );
    entireTrip.click();
    await browser.pause(5000);
...
*/
  ...
  it(" Advisor_Bookings_TC024", async () => {
    ...
    await browser.pause(5000);
    //check fileds
    const reportduptitle = await $(
      '//*[@id="headlessui-dialog-title-:ru:"]/div/div'
    );
    if (!reportduptitle.isDisplayed()) {
      throw new Error("title not displayed");
    }
    const notes = await $(
      '//*[@id="headlessui-dialog-panel-:rt:"]/div[1]/div/div[2]/p'
    );
    if (!reportduptitle.isDisplayed()) {
      throw new Error("title not displayed");
    }
    ...
    const confirm = await $(
      "//button[normalize-space()='Yes, this is a duplicate']"
    );
    ...
    //type notes
    const cmt = await $(
      '//*[@id="headlessui-dialog-panel-:rt:"]/div[1]/div/div[2]/textarea'
    );
    await cmt.setValue("testing1", Key.ENTER);
    await browser.pause(5000);
  ...
  it(" Advisor_Bookings_TC025", async () => {
    ...
    const howcan = await $(
      '//*[@id="headlessui-dialog-panel-:r11:"]/div[1]/div/div/p'
    );
    if (!howcan.isDisplayed()) {
      throw new Error("title not displayed");
    }
    const txbox = await $(
      "//textarea[@placeholder='Any additional context about this booking...']"
    );
    ...
    const thnku = await $(
      '//*[@id="headlessui-dialog-panel-:r1h:"]/div/div/h4'
    );
    if (!thnku.isDisplayed()) {
      throw new Error("thnku is not displayed");
    }
  ...
  it(" Advisor_Bookings_TC026", async () => {
    ...
    filter.click();
    //todays date
    const todaysdate = await $(
      "//div[@class='react-datepicker__day react-datepicker__day--005 react-datepicker__day--today']"
    );
    todaysdate.click();
    //apply
