...
describe("fora-homepgae", () => {
  ...
  const scheduleEvent=await $("//button[contains(span/text(), 'Schedule Event')]")
  await $(scheduleEvent).click()
  
  await browser.pause(10000)
   
  //heading after scheduling a call 
  const postSchedulingTitle=await $("//h2[contains(text(), 'You are scheduled for an Advisor kickoff call')]")
  await $(postSchedulingTitle).toHaveText("You are scheduled for an Advisor kickoff call")
  });
})